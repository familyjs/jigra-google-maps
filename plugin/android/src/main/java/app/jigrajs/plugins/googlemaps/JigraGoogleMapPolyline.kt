...
class JigraGoogleMapPolyline(fromJSONObject: JSONObject) {
    var path: MutableList<LatLng> = mutableListOf<LatLng>()
    var styleSpans: MutableList<JigraGoogleMapsStyleSpan> = mutableListOf<JigraGoogleMapsStyleSpan>()
            ...
            for (i in 0 until styleSpanArray.length()) {
                ...
                if (obj.has("color")) {
                    ...
                    if (obj.has("segments")) {
                        ...
                        styleSpans.add(JigraGoogleMapsStyleSpan(Color.parseColor(color), segments))
                    } else {
                        styleSpans.add(JigraGoogleMapsStyleSpan(Color.parseColor(color), null))
                    }
                }
            }
        }
